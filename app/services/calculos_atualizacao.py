...
def calcular_atualizacao(payload: dict) -> dict:
    ...
    multa_tipo       = payload.get("multa_tipo") or "valor"
    multa_percentual = parse_decimal(payload.get("multa_percentual"))
    multa_valor_raw  = parse_money(payload.get("multa_valor"))

    # Base para multa = corrigido + juros (mesma base dos honorários)
    base_multa = _q(total_corrigido + total_juros)

    if multa_tipo == "percentual" and multa_percentual > 0:
        multa = _q(base_multa * (multa_percentual / Decimal("100")))
        multa_label = f"{multa_percentual:.2f}% sobre o valor corrigido"
    else:
        multa = multa_valor_raw
        multa_label = ""

    # --- honorários: percentual ou valor fixo ---
    honorarios_tipo       = payload.get("honorarios_tipo") or "valor"
    honorarios_percentual = parse_decimal(payload.get("honorarios_percentual"))
    honorarios_valor_raw  = parse_money(payload.get("honorarios_valor"))

    # Base para cálculo percentual = corrigido + juros (antes de multa/custas)
    base_honorarios = _q(total_corrigido + total_juros)

    if honorarios_tipo == "percentual" and honorarios_percentual > 0:
        honorarios = _q(base_honorarios * (honorarios_percentual / Decimal("100")))
        honorarios_label = f"{honorarios_percentual:.2f}% sobre o valor corrigido"
    else:
        honorarios = honorarios_valor_raw
        honorarios_label = ""
